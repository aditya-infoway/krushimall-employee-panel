...
const Accessories = () => {
  ...
  const getAccessories = async () => {
    try {
      setLoading(true);
      const response = await apiHelper.get("/storemanager/accessories");
  ...
  const performDelete = async () => {
    ...
    try {
      if (isBulkDelete) {
        await Promise.all(
          selectedIds.map((id) => apiHelper.delete(`/storemanager/accessories/${id}`)),
        );
        ...
        await apiHelper.delete(`/storemanager/accessories/${deleteTargetId}`);
  ...
  const handleToggleStatus = async (id: number) => {
    ...
    try {
      await apiHelper.put(`/storemanager/accessories/${id}`, { status: newStatus });
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await apiHelper.put(`/storemanager/accessories/${editId}`, payload);
        toast.success("Item updated successfully!");
      } else {
        await apiHelper.post("/storemanager/accessories", payload);
