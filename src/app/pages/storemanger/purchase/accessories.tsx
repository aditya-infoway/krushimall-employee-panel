// src/app/pages/purchase/accessories.tsx
...
interface AccessoriesPurchaseRegisterProps {
  rows?: AccessoriesPurchaseRegisterRow[];
  onAddPurchase?: () => void;
  onEditRow?: (row: AccessoriesPurchaseRegisterRow) => void;
  onDeleteRow?: (row: AccessoriesPurchaseRegisterRow) => void;
}
...
const statusColors: Record<AccessoriesPurchaseRegisterRow["status"], string> = {
  Pending:
...
const fetchAccessoriesPurchases = async () => {
  try {
    const res = await apiHelper.get("/storemanager/accessories-purchase");
...
useEffect(() => {
  ...
  const handleAddPurchase = () => {
    navigate("/purchase/accessories/add");
    onAddPurchase?.();
  };
...
const performDelete = async () => {
  ...
  try {
    if (isBulkDelete) {
      await Promise.all(selectedIds.map((id) => apiHelper.delete(`/storemanager/accessories-purchase/${id}`)));
      ...
      await apiHelper.delete(`/storemanager/accessories-purchase/${deleteTargetId}`);
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900 md:text-2xl dark:text-white">
            Accessories Purchase Register
          </h1>
          <p className="dark:text-dark-300 mt-1 text-sm text-gray-500">
            Manage all accessories purchase transactions from here
          </p>
        </div>
    ...
    Add Purchase
      ...
      <div className="dark:bg-dark-800 dark:border-dark-700 rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <Table
            hoverable
            className="w-full min-w-400 text-left [&_.table-th]:font-semibold"
          >
...
export default AccessoriesPurchaseRegister;
