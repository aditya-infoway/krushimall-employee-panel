...
const Account = () => {
  ...
  const performDelete = async () => {
    ...
    try {
      if (isBulkDelete) {
       await Promise.all(
  selectedIds.map((id) =>
    apiHelper.delete(`/accountant/accounts/${id}`)
  ),
);
      ...
      await apiHelper.delete(`/accountant/accounts/${deleteTargetId}`);
  ...
  const getAccounts = async () => {
    try {
      const response = await apiHelper.get("/accountant/accounts");
