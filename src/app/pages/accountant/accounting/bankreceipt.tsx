...
// Payment Type filter options — dynamically built from actual rows data
const paymentTypeFilterOptions = [
  { id: "All", name: "All Types" },
  ...Array.from(
    new Set(rows.map((r) => r.paymentType).filter(Boolean)),
...
const getBankReceipts = async () => {
  try {
    const res = await apiHelper.get("/accountant/bank-receipt");
...
const getVoucherNo = async () => {
 ...
 const handleSubmit = async () => {
  ...
  try {
    ...
    if (editId) {
      await apiHelper.put(`/accountant/bank-receipt/${editId}`, payload);
      toast.success("Bank receipt updated successfully!");
    } else {
      await apiHelper.post("/accountant/bank-receipt", payload);
...
const downloadExcel = async () => {
  try {
    const blob = await apiHelper.getBlob(
      "/accountant/bank-receipt/export/excel"
    );
 ...
 const handlePrint = async (item: BankReceipt) => {
  try {
    const blob = await apiHelper.getBlob(`/accountant/bank-receipt/${item.id}/print`);
    const url = window.URL.createObjectURL(blob);
  ...
  <div>
    ...
    <Listbox
      ...
      value={
        paymentTypeFilterOptions.find(
          (o) => o.id === filterPaymentType,
        ) || paymentTypeFilterOptions[0]
      }
      onChange={(opt: any) => setFilterPaymentType(opt.id)}
      displayField="name"
    />
  </div>
              ...
              <div className="grow space-y-5 overflow-y-auto p-5">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <Combobox
                     data={oppAccounts.filter(
    (a: any) => a.value !== form.bankAccount?.value
  )}
                      displayField="label"
