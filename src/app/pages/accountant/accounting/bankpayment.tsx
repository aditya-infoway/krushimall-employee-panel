...
export default function BankPayment() {
  ...
  const getBankPayments = async () => {
    try {
      const res = await apiHelper.get("/accountant/bank-payment");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editId) {
        await apiHelper.put(`/accountant/bank-payment/${editId}`, payload);
        toast.success("Bank payment updated successfully!");
      } else {
        await apiHelper.post("/accountant/bank-payment", payload);
  ...
  const downloadExcel = async () => {
    try {
      const blob = await apiHelper.getBlob("/accountant/bank-payment/export/excel");
